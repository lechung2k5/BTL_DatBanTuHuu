		...
		try (Connection con = ConnectDB.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					...
					if (nv == null) {
						System.err.println("Lỗi dữ liệu: Tài khoản " + tenDangNhap
								+ " hợp lệ nhưng không tìm thấy Nhân viên " + maNV);
						return null;
					}
					...
					tk = new TaiKhoan(rs.getString("tenDangNhap"), null, // Không lưu mật khẩu vào đối tượng sau khi xác
																			// thực
							vaiTro, nv);
				}
			}
		...
		try (Connection con = ConnectDB.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					NhanVien nv = nhanVienDAO.getNhanVienTheoMa(maNV);
					if (nv == null)
						return null; // Quan trọng: Kiểm tra nếu NV không tồn tại
					String vaiTroString = rs.getString("vaiTro");
					VaiTro vaiTro = VaiTro.fromString(vaiTroString);
					tk = new TaiKhoan(rs.getString("tenDangNhap"), rs.getString("matKhau"), // Hàm này có thể vẫn cần
																							// mật khẩu tùy mục đích
							vaiTro, nv);
				}
			}
		} catch (SQLException e) {
