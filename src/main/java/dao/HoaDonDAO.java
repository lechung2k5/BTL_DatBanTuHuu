...
public class HoaDonDAO {
	...
	public List<HoaDon> getAllHoaDon() {
		...
		try (Connection conn = ConnectDB.getConnection();
			...
			while (rs.next()) {
				...
				hd.setGioVao((tsGioVao != null) ? tsGioVao.toLocalDateTime() : null);
				Timestamp tsGioRa = rs.getTimestamp("gioRa");
				hd.setGioRa((tsGioRa != null) ? tsGioRa.toLocalDateTime() : null);
	...
	public List<ChiTietHoaDon> getChiTietHoaDon(String maHD) {
		...
		try (Connection conn = ConnectDB.getConnection(); PreparedStatement pstmt = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = pstmt.executeQuery()) {
				while (rs.next()) {
					chiTietList.add(new ChiTietHoaDon(rs.getString("tenMon"), rs.getInt("soLuong"),
	...
	// --- HÀM MỚI ---
	/**
	 * Lấy thông tin chi tiết đầy đủ của một hóa đơn dựa vào mã hóa đơn. Join với
	 * KhachHang, Ban, NhanVien.
	 * 
