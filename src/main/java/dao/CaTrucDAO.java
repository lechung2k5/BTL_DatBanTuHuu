...
public class CaTrucDAO {
	...
	public List<CaTruc> layCaTrucTrongTuan(LocalDate ngayDauTuan) { // Giữ nguyên hàm gốc
		...
		String sql = "SELECT CT.*, NV.tenNV " + "FROM CaTruc CT " + "JOIN NhanVien NV ON CT.maNV = NV.maNV "
		...
		try (Connection con = ConnectDB.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					...
					CaTruc caTruc = new CaTruc(rs.getString("maCa"), rs.getDate("ngay").toLocalDate(),
							rs.getTime("gioBatDau").toLocalTime(), rs.getTime("gioKetThuc").toLocalTime(), nv);
	...
	/**
	 * 🔥 HÀM MỚI (Đã thêm trước đó): Lấy tất cả các ca trực của MỘT NHÂN VIÊN trong
	 * tuần (cho Dashboard).
	 * 
	 * @param ngayDauTuan Ngày thứ Hai của tuần cần lấy dữ liệu.
