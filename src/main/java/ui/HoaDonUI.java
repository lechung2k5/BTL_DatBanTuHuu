...
public class HoaDonUI {
	...
	private List<KeyCodeCombination> shortcuts = new java.util.ArrayList<>();
	...
	private void initialize() {
		...
		// ✅ QUẢN LÝ PHÍM TẮT ĐÚNG CÁCH
		tableHoaDon.sceneProperty().addListener((obs, oldScene, newScene) -> {

			// 🛑 Nếu đang có scene cũ → GỠ NGAY shortcut khỏi scene cũ
			if (oldScene != null) {
				oldScene.getAccelerators().clear();
			}

			// ✅ Nếu scene mới hiển thị → GẮN shortcut trang này
			if (newScene != null) {
				disableShortcuts(); // Tránh nhân đôi
				enableShortcuts();
			}
		}); // ✅ Đóng listener
	...
	public void enableShortcuts() {
		if (tableHoaDon.getScene() == null)
			return;
		Scene scene = tableHoaDon.getScene();
		...
		// Ctrl + ←
		KeyCodeCombination scPrev = new KeyCodeCombination(KeyCode.LEFT, KeyCombination.CONTROL_DOWN);
		scene.getAccelerators().put(scPrev, () -> {
			int i = tableHoaDon.getSelectionModel().getSelectedIndex();
			if (i > 0)
				tableHoaDon.getSelectionModel().select(i - 1);
		});
		shortcuts.add(scPrev);

		// Ctrl + P
		KeyCodeCombination scPrint = new KeyCodeCombination(KeyCode.P, KeyCombination.CONTROL_DOWN);
		scene.getAccelerators().put(scPrint, () -> btnInHoaDon.fire());
		shortcuts.add(scPrint);

		// Ctrl + E
		KeyCodeCombination scExport = new KeyCodeCombination(KeyCode.E, KeyCombination.CONTROL_DOWN);
		scene.getAccelerators().put(scExport, () -> btnXuatExcel.fire());
	...
	public void disableShortcuts() {
		...
		tableHoaDon.setOnKeyPressed(null);
	}
