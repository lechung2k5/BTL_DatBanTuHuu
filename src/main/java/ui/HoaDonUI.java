...
public class HoaDonUI {
    ...
    @FXML private TableColumn<HoaDon, String> colNgay; // Kiểu String vì hiển thị đã format
    @FXML private TableColumn<HoaDon, PTTThanhToan> colHinhThucThanhToan;
    @FXML private TableColumn<HoaDon, String> colSoDienThoaiKH; // Dùng getter mới
    ...
    @FXML private Label lblMaKHValue; // Sẽ hiển thị SĐT
    ...
    private final DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm"); // Format cho LocalDateTime
                ...
                btnXem.setOnAction(event -> {
                    ...
                    if (hoaDon != null) {
                        hienThiThongTinHoaDon(hoaDon);
                         // Có thể thêm: tableHoaDon.getSelectionModel().select(hoaDon); // Tự động select dòng khi nhấn Xem
                    }
                });
            }
            @Override
    ...
    /**
     * Hiển thị thông tin chi tiết của một hóa đơn lên panel bên phải.
     * Đã cập nhật để dùng getters mới và định dạng LocalDateTime.
     */
    private void hienThiThongTinHoaDon(HoaDon hoaDon) {
        ...
        lblGioVaoValue.setText(hoaDon.getGioVao() != null ? hoaDon.getGioVao().format(timeFormatter) : "N/A"); // Chỉ hiển thị giờ:phút
        lblGioRaValue.setText(hoaDon.getGioRa() != null ? hoaDon.getGioRa().format(timeFormatter) : "N/A"); // Chỉ hiển thị giờ:phút
