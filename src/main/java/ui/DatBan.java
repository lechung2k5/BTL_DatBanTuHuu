...
public class DatBan implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		if (comboFilter != null) {
			comboFilter.setItems(FXCollections.observableArrayList("Tất cả", "Đang phục vụ", "Đã đặt"));
		...
		// =========================================================
		// BƯỚC 3: THÊM PHÍM TẮT
		// =========================================================
		Platform.runLater(() -> { // Đảm bảo scene đã được gắn vào Stage
			Scene scene = rootPane.getScene(); // rootPane là BorderPane hoặc Pane chính của DatBan.fxml
			...
			scene.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
				...
				// Ctrl + P → In hóa đơn
				else if (new KeyCodeCombination(KeyCode.P, KeyCombination.CONTROL_DOWN).match(event)) {
					if (!btnInHoaDon.isDisabled()) {
						btnInHoaDon.fire();
					}
					event.consume();
				}
				...
				// Ctrl + B → Đổi bàn
					...
					if (!btnDoiBan.isDisabled()) {
						btnDoiBan.fire();
					}
					event.consume();
				}

				// Ctrl + W → Tách bàn ✅ cập nhật theo yêu cầu mới
				else if (new KeyCodeCombination(KeyCode.W, KeyCombination.CONTROL_DOWN).match(event)) {
					if (!btnTachBan.isDisabled()) {
						btnTachBan.fire();
					}
					event.consume();
				}
				...
				// Ctrl + G → Gộp bàn
					...
					if (!btnGopBan.isDisabled()) {
						btnGopBan.fire();
					}
					event.consume();
				}

				// Esc → Hủy bàn ✅ yêu cầu mới
				else if (event.getCode() == KeyCode.ESCAPE) {
					if (!btnHuyBan.isDisabled()) {
						btnHuyBan.fire();
					}
					event.consume();
				}

				// F5 → Làm mới dữ liệu bàn
				else if (event.getCode() == KeyCode.F5) {
					loadTableGrids(); // nếu bạn có load lại bàn
					loadBookingCards(); // load lại danh sách đặt bàn
					event.consume();
				}
			});
		});
	...
	/**
	 * 🔥 HÀM SỬA CUỐI CÙNG: Xử lý nút In Hóa đơn (In Hóa đơn TẠM TÍNH). Cho phép in
	 * nếu HĐ đang ở trạng thái ĐANG SỬ DỤNG hoặc ĐÃ ĐẶT (Chưa thanh toán).
	 */
	@FXML
		...
		new Thread(() -> {
			...
			try {
				...
				if (job.printDialog(attr)) {
					...
					org.apache.pdfbox.printing.PDFPrintable printableData = new org.apache.pdfbox.printing.PDFPrintable(
							document);
					...
					Platform.runLater(() -> {
						showAlert(Alert.AlertType.INFORMATION, "In Hóa đơn",
								"Đã gửi lệnh in cho Hóa đơn " + currentHoaDon.getMaHD() + " thành công.");
					});
				} else {
				...
				Platform.runLater(() -> showAlert(Alert.AlertType.ERROR, "Lỗi In",
						"Lỗi trong quá trình in ấn: " + e.getMessage()));
			} catch (Exception e) {
				e.printStackTrace();
				Platform.runLater(
						() -> showAlert(Alert.AlertType.ERROR, "Lỗi Hệ thống", "Lỗi khi tạo PDF: " + e.getMessage()));
			} finally {
				if (document != null) {
					try {
	...
	/**
	 * 🔥 HÀM CUỐI CÙNG: Tạo đối tượng PDDocument theo MẪU ẢNH HÓA ĐƠN. ĐÃ FIX: Điều
	 * chỉnh căn chỉnh cột Tên món (làm hẹp) và các cột giá trị (thêm padding) để
	 * tối ưu khoảng cách.
	 * 
	...
	private PDDocument createReceiptPdf(HoaDon hd) throws IOException {
		...
		double tongTienMonAn = monAnList.stream().mapToDouble(order -> order.getDonGia() * order.getSoLuong()).sum();
		...
		String khachHangStr = (hd.getKhachHang() != null && hd.getKhachHang().getSoDT() != null)
		...
		try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
			...
			for (int i = 0; i < monAnList.size(); i++) {
				...
				contentStream.setTextMatrix(
						org.apache.pdfbox.util.Matrix.getTranslateInstance(colSL - slWidth + 15, currentY));
				...
				contentStream.setTextMatrix(
						org.apache.pdfbox.util.Matrix.getTranslateInstance(colDonGia - dgWidth + 15, currentY));
				...
				contentStream.setTextMatrix(
						org.apache.pdfbox.util.Matrix.getTranslateInstance(colThanhTien - ttWidth + 15, currentY));
			...
			String uuDaiStr = (hd.getKhuyenMai() > 0) ? ("-" + currencyFormatter.format(hd.getKhuyenMai()) + " VNĐ")
					: "0 VNĐ";
	...
	/**
	 * 🔥 HÀM SỬA CUỐI CÙNG CHO MOMO: Mở Popup hiển thị QR Thanh toán (Sử dụng LOGO
	 * và BIN BVBank). * SẼ CẦN KẾT NỐI INTERNET ĐỂ TẢI ẢNH QR.
	 */
			...
			if (maHDCanThanhToan != null) {
				...
				if (datBanDAO.thanhToanHoaDon(maHDCanThanhToan, pttt, maNV)) {
					...
					if (hdDaThanhToan != null) {
						...
						// Cần gọi lại handleThanhToan để cập nhật các Labels trên Receipt với Giờ Ra
						// mới
						handleThanhToan();
	 ...
	 * Panel). Giữ nguyên các thông tin HĐ và kích hoạt nút In Hóa đơn.
	 */
	...
	/**
	 * 🔥 HÀM MỚI: Vô hiệu hóa các nút thao tác nghiệp vụ (Panel Giữa). Đảm bảo các
	 * nút Sửa/Đổi/Gộp/Tách/Hủy không dùng được sau khi thanh toán.
	 */
	...
	/**
	 * 🔥 HÀM SỬA CUỐI CÙNG: Mở Popup hiển thị QR Thanh toán Ngân hàng (Thêm màu cho
	 * nút Hủy/Xác nhận). * SẼ CẦN KẾT NỐI INTERNET ĐỂ TẢI ẢNH QR.
	 */
	...
	/**
	 * 🔥 HÀM SỬA CUỐI CÙNG: Tạo URL Quicklink của MoMo (ĐÃ FIX LỖI BIÊN DỊCH
	 * ENCODE). Sử dụng cú pháp rút gọn cho MoMo để đảm bảo tính ổn định.
	 */
	private String createMomoQrUrl(String maHD, double amount) {
		...
		String momoPayLink = String.format("https://payment.momo.vn/pay?phone=%s&amount=%d&note=%s", YOUR_MOMO_PHONE,
				(int) Math.ceil(amount), rawContent // Nội dung chưa mã hóa URL
		);
		...
		try {
			...
			encodedUrl = "https://api.qrserver.com/v1/create-qr-code/?data="
					+ java.net.URLEncoder.encode(encodedMomoPayLink, "UTF-8") + "&size=250x250";
		} catch (java.io.UnsupportedEncodingException e) {
	...
	/**
	 * 🔥 HÀM MỚI: Tải ảnh QR Code từ một URL. KHÔNG CẦN THƯ VIỆN ZXING NỮA.
	 *
	...
	private Image generateQrCodeImage(String qrData, int size) {
		try {
			...
			BitMatrix bitMatrix = new MultiFormatWriter().encode(qrData, BarcodeFormat.QR_CODE, size, size, hints);
			...
			showAlert(Alert.AlertType.ERROR, "Lỗi tạo QR",
					"Không thể tạo mã QR. Vui lòng kiểm tra thư viện tạo QR (ZXing) và dữ liệu QR Code.");
	...
	private void moPopupThanhToanTienMat() {
		...
		inputContainer.getChildren().addAll(createInputGridRow("Tiền khách đưa:", txtTienKhachDua, true),
	...
	/**
	 * Hàm helper createInputGridRow (Đã sửa ở bước trước) Vui lòng đảm bảo bạn có
	 * hàm này trong class DatBan.java
	 */
	...
	private void handlePromoSelection(String selectedName) {
		if (selectedName == null || selectedName.equals("Không áp dụng")) {
			...
			selectedUuDai = dsUuDaiDangApDung.stream().filter(ud -> ud.getTenUuDai().equals(simpleName)).findFirst()
					.orElse(null);
		}
	...
	private void loadPromoComboBox() {
		...
		dsUuDaiDangApDung = allUuDai.stream().filter(ud -> ud.getTrangThai().equals("Đang áp dụng"))
	...
	/**
	 * 🔥 HÀM MỚI: Mở Popup Tách Bàn (TachBanPopup.fxml). * Đảm bảo: 1. Hóa đơn gốc
	 * (currentHoaDon) đã được chọn. 2. Hóa đơn phải đang ở trạng thái 'Đang sử
	 * dụng' (DANG_SU_DUNG) hoặc 'HoaDonTam'.
	 */
	...
	private void openDoiBanPopup() {
		...
		try {
			...
			popupController.setInitialData(currentHoaDonGocVaPhu, datBanDAO, this); // Truyền list HĐ, DAO và controller
																					// chính
	...
	/**
	 * 🔥 HÀM XỬ LÝ NGHIỆP VỤ TÁCH BÀN Tạo một hóa đơn mới (với trạng thái
	 * HOADONTAM) dựa trên hóa đơn cũ, và gán cho một bàn TẠM THỜI (NULL).
	 * 
	...
	/**
	 * 🔥 XỬ LÝ LƯU THÔNG TIN ĐẶT HÀNG / KHÁCH ĐẾN QUÁN === ĐÃ SỬA LOGIC (Gốc - Phụ)
	 * ĐỂ XỬ LÝ NHIỀU BÀN (Dùng maHDGoc) === Chỉ tạo 1 Hóa Đơn GỐC (có món, có cọc)
	 * và N Hóa Đơn PHỤ (không món, không cọc).
	 */
	...
	private void handleXacNhanBan() {
		...
		showAlert(Alert.AlertType.INFORMATION, "Thành công",
				"Đã xác nhận bàn. Tiền cọc đã được tính và áp dụng. Vui lòng nhập thông tin.");
	...
	/**
	 * Tính toán trạng thái hiển thị của bàn dựa trên MỘT thời điểm kiểm tra. Logic:
	 * Ưu tiên trạng thái ĐANG PHỤC VỤ (CAM), còn lại áp dụng logic ĐÃ ĐẶT (ĐỎ). ===
	...
	public TrangThaiBan getTrangThaiHienThi(Ban banGoc, LocalTime thoiGianKiemTra) {
		// [Copy-paste toàn bộ nội dung của hàm getTrangThaiHienThi đã được sửa ở bước
		// trước vào đây]
		// (Để giữ cho phản hồi này ngắn gọn, hãy sử dụng lại mã của hàm
		// getTrangThaiHienThi đã gửi trước đó)

		// 1. Tìm HĐ đang hoạt động (Dat, DangSuDung, HoaDonTam) sớm nhất cho bàn này
		Optional<HoaDon> hdDangCho = dsHoaDonDatTrongNgay.stream().filter(
				hd -> hd.getBan() != null && hd.getBan().getMaBan().equals(banGoc.getMaBan()) && hd.getGioVao() != null)
	...
	public void loadTableGrids() {
		...
		System.out.println(
				"LOG loadTableGrids: Đã tải lại " + dsHoaDonDatTrongNgay.size() + " HĐ đang chờ cho logic tô màu.");
	...
	/**
	 * Xử lý khi người dùng bấm chọn/bỏ chọn một bàn trên sơ đồ. === ĐÃ SỬA: Logic
	 * Cảnh báo 8 tiếng khi chọn bàn ===
	 */
	private void handleChonBan(Ban ban, Button currentButton) {
		...
		Optional<HoaDon> datGanNhat = dsHoaDonDatTrongNgay.stream().filter(
				hd -> hd.getBan() != null && hd.getBan().getMaBan().equals(ban.getMaBan()) && hd.getGioVao() != null)
				.filter(hd -> hd.getTrangThai() != null && hd.getTrangThai().getDbValue().equals("Dat"))
				.min(Comparator.comparing(hd -> hd.getGioVao()));
	...
	private void updateSelectionLabels() {
		...
		String banNames = selectedBanList.stream().map(Ban::getMaBan).collect(Collectors.joining(", "));
	...
	public void loadBookingCards() {
		...
		try {
			...
			List<HoaDon> filteredList = allPendingBookings.stream()
					...
					.filter(hd -> {
						...
						if (selectedStatus != null && !selectedStatus.equals("Tất cả")) {
							...
							if (selectedStatus.equals("Đang phục vụ")) {
								statusMatch = hd.getTrangThai() != null && (hd.getTrangThai().getDbValue()
										.equals(TrangThaiHoaDon.DANG_SU_DUNG.getDbValue())
										|| hd.getTrangThai().getDbValue()
												.equals(TrangThaiHoaDon.HOA_DON_TAM.getDbValue()));
							} else {
						...
						if (searchQuery != null && !searchQuery.trim().isEmpty()) {
							...
							searchMatch = hd.getKhachHang() != null && hd.getKhachHang().getSoDT() != null
			...
			if (filteredList.isEmpty()) {
				...
				// Chỉ hiển thị các HĐ GỐC trong danh sách filter (HĐ Phụ sẽ được hiển thị qua
				// HĐ Gốc)
				List<HoaDon> hdGocFilter = filteredList.stream().filter(hd -> hd.getMaHDGoc() == null)
						.collect(Collectors.toList());
	...
	/**
	 * 🔥 HÀM MỚI: Load thông tin của Hóa đơn lên giao diện chính === ĐÃ NÂNG CẤP
	 * (Gốc - Phụ) + TÔ MÀU BÀN TRÊN SƠ ĐỒ === Sẽ tìm và hiển thị tất cả các bàn
	 * (Gốc và Phụ) liên quan đến HĐ này. Sẽ tô màu đỏ các bàn tương ứng trên sơ đồ.
	 */
	private void loadHoaDonToMainInterface(HoaDon hd) {
		System.out.println(
				"LOG: Đang tải Hóa đơn " + (hd.getMaHD() != null ? hd.getMaHD() : "Mới") + " lên giao diện chính.");
		...
		if (hd.getMaHDGoc() != null) {
			showAlert(Alert.AlertType.INFORMATION, "Thông báo",
					"Bạn đã chọn Hóa đơn Phụ. Đang tải Hóa đơn Gốc liên quan...");
		...
		String tatCaBan = currentHoaDonGocVaPhu.stream().map(h -> {
			String tenBan = (h.getBan() != null) ? h.getBan().getMaBan() : "N/A";
			if (h.getMaHDGoc() == null)
				return tenBan + " (Gốc)";
			return tenBan;
		...
		TrangThaiBan trangThaiCanTo = trangThaiHdGoc == TrangThaiHoaDon.DANG_SU_DUNG ? TrangThaiBan.DANG_SU_DUNG // Màu
																													// cam
																													// nếu
																													// đang
																													// phục
	...
	/**
	 * Tạo một VBox card hiển thị thông tin tóm tắt của Hóa đơn. === ĐÃ SỬA: Hiển
	 * thị danh sách bàn duy nhất, bỏ chữ "(Gốc)" ===
	 */
	private VBox createBookingCard(HoaDon hd) {
		...
		try {
			...
			Set<String> uniqueBanSet = allRelatedHDs.stream().filter(h -> h.getBan() != null) // Lọc bỏ HĐ không có bàn
			...
			danhSachBanDayDu = uniqueBanSet.isEmpty() ? "N/A" : String.join(", ", new TreeSet<>(uniqueBanSet)); // Sắp
																												// xếp
																												// A-Z
	...
	private void openChiTietDatBanPopup(HoaDon hd) {
		...
		try {
			...
			// Gán Controller cha (this) vào UserData để các nút trong Popup có thể gọi lại
			// DatBan.java
			...
			showAlert(Alert.AlertType.ERROR, "Lỗi Hệ thống",
					"Đã xảy ra lỗi không xác định khi mở Popup: " + e.getMessage());
		}
	}
				...
				if (empty || getItem() == null) {
					...
					if (hinhAnhBytes != null && hinhAnhBytes.length > 0) {
						try {
							...
							System.err.println("❌ LỖI RUNTIME LOAD ẢNH: "
									+ getTableView().getItems().get(getIndex()).getTenMon() + " - " + e.getMessage());
	...
	private void handleChonMon(MonAn mon) {
		Optional<MonOrder> existingOrder = monOrderList.stream().filter(o -> o.getMaMon().equals(mon.getMaMon()))
				.findFirst();
	...
	private void calculateTotal() {
		double tongTienMonAn = monOrderList.stream().mapToDouble(order -> order.getDonGia() * order.getSoLuong()).sum();
	...
	public void clearFormDatBan() {
		...
		for (Button btn : selectedButtonList) { //
			...
			Ban banGoc = selectedBanList.stream()
					.filter(b -> selectedButtonList.indexOf(btn) == selectedBanList.indexOf(b)).findFirst()
					.orElse(null);
			if (banGoc != null) {
	...
	private void handleTimTenKhachHang(String sdt) {
		...
		try {
			...
			if (khachTimDuoc != null && khachTimDuoc.getMaKH() != null && khachTimDuoc.getTenKH() != null
					&& !khachTimDuoc.getTenKH().equals("Khách vãng lai")) {
	...
	private void handleTimBanTrong() {
		...
		try {
			...
			System.out.println("  DEBUG: Đã cập nhật dsHoaDonDatTrongNgay cho ngày " + ngay + ". Số lượng: "
					+ dsHoaDonDatTrongNgay.size());
			...
			for (Map<String, Object> banInfo : allBanInfo) {
				...
				System.out.println("    - Processing Table: " + ban.getMaBan() + " | Is Available (DAO): " + isAvailable
						+ " | 4/8h Logic Status: " + trangThaiTheoLogic48);
				...
				if (trangThaiTheoLogic48 != TrangThaiBan.TRONG) {
					...
					System.out.println(
							"      -> Priority 1: Assigning status based on 4/8h logic (BUSY): " + finalStatus);
				}
			...
			showAlert(Alert.AlertType.ERROR, "Lỗi định dạng",
					"Giờ nhập không hợp lệ. Vui lòng nhập theo định dạng HH:mm (ví dụ: 14:30).");
		} catch (Exception ex) {
			ex.printStackTrace();
			showAlert(Alert.AlertType.ERROR, "Lỗi",
					"Không thể tìm bàn trống. Kiểm tra dữ liệu đầu vào hoặc kết nối DB.");
	...
	private void loadTableGridsBaseBasedOnCurrentBooking(List<HoaDon> cumHoaDonDangXem) {
		...
		Set<String> maBanDangXemSet = cumHoaDonDangXem.stream().filter(hd -> hd.getBan() != null)
				.map(hd -> hd.getBan().getMaBan()).collect(Collectors.toSet());
		HoaDon hoaDonGoc = cumHoaDonDangXem.stream().filter(hd -> hd.getMaHDGoc() == null).findFirst().orElse(null);
		TrangThaiBan trangThaiCanTo = (hoaDonGoc != null
				&& TrangThaiHoaDon.fromDbValue(hoaDonGoc.getTrangThai().getDbValue()) == TrangThaiHoaDon.DANG_SU_DUNG)
		...
		for (Ban ban : tatCaBan) {
			...
			if (maBanDangXemSet.contains(ban.getMaBan())) {
				...
				// Với các bàn khác, lấy trạng thái theo giờ hiện tại (logic cũ của
				// loadTableGrids)
	...
	/**
	 * 🔥 XỬ LÝ SỬA HÓA ĐƠN (Thông tin KH, Tiền cọc, Danh sách món) ĐÃ SỬA: Loại bỏ
	 * hoàn toàn hộp thoại xác nhận. Việc lưu được thực hiện tự động.
	 */
	...
	private void handleHuyBan() {
		...
		Optional<ButtonType> result = showAlertConfirm("XÁC NHẬN HỦY",
				"Bạn có chắc muốn HỦY toàn bộ cụm Hóa đơn Gốc " + currentHoaDon.getMaHD()
						+ " và các Hóa đơn Phụ liên quan?\nBàn liên quan sẽ được trả trống. KHÔNG THỂ HOÀN TÁC!");
		...
		if (result.isPresent() && result.get() == ButtonType.OK) {
			try {
				...
				for (HoaDon hdToHuy : allRelatedHDs) {
					...
					datBanDAO.capNhatTrangThaiHoaDon(hdToHuy.getMaHD(), TrangThaiHoaDon.DA_HUY.getDbValue(), true); // true
																													// =
																													// set
																													// giờ
																													// ra
					...
					if (hdToHuy.getBan() != null) {
						...
						System.out.println(
								"LOG: Đã hủy HD " + hdToHuy.getMaHD() + " và trả bàn " + hdToHuy.getBan().getMaBan());
					}
				}

				showAlert(Alert.AlertType.INFORMATION, "Thành công",
						"Đã hủy toàn bộ cụm Hóa đơn gốc " + currentHoaDon.getMaHD() + " và các hóa đơn phụ liên quan.");
	...
	private void handleThanhToan() {
		...
		if (vboxReceipt != null) {
			...
			currentHoaDon.setTongCongMonAn(
					monOrderList.stream().mapToDouble(order -> order.getDonGia() * order.getSoLuong()).sum());
	...
	/**
	 * 🔥 HELPER MỚI: Tải lại danh sách HĐ đang chờ cho logic tô màu. Cần phải
	 * PUBLIC để DoiBanPopupController có thể gọi.
	 */
	...
	/**
	 * HELPER: Kiểm tra xem bàn có nằm trong cụm HĐ đang xem (HĐ Gốc/Phụ) không. Cần
	 * phải PUBLIC để TachBanPopupController và DoiBanPopupController có thể gọi.
	 */
	public boolean isBanInCurrentBooking(Ban ban) {
		if (ban == null || currentHoaDonGocVaPhu.isEmpty())
			return false;
		return currentHoaDonGocVaPhu.stream().filter(hd -> hd.getBan() != null)
