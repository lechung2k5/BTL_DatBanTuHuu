...
/**
 * Controller cho màn hình Đặt Bàn. Đã cập nhật để đồng bộ với entity HoaDon
 * mới.
 */
public class DatBan implements Initializable {
	...
	@FXML
	private VBox vboxRightPanel;
	@FXML
	private VBox vboxMenuGoiMon;
	@FXML
	private ScrollPane vboxReceipt;
	@FXML
	private HBox hboxDanhMucToggles;
	@FXML
	...
	@FXML
	private TableView<MonAnViewModel> tblMonAn; // Sử dụng ViewModel
	@FXML
	private TableColumn<MonAnViewModel, String> colTenMon;
	@FXML
	private TableColumn<MonAnViewModel, Image> colHinhAnh; // Sửa kiểu thành Image
	@FXML
	private TableColumn<MonAnViewModel, Number> colGia;
	@FXML
	private TableColumn<MonAnViewModel, Void> colChon;
	@FXML
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		if (btnInHoaDon != null)
			btnInHoaDon.setOnAction(e -> handleInHoaDon()); // Gán sự kiện nút In
		...
		// Search Listeners
		if (txtTimMon != null)
			txtTimMon.textProperty().addListener((o, ov, nv) -> handleTimMon(nv));
		if (txtSoDienThoai != null)
			txtSoDienThoai.focusedProperty().addListener((o, ov, nv) -> {
				if (!nv)
		...
		monOrderList
				.addListener((javafx.collections.ListChangeListener.Change<? extends MonOrder> c) -> calculateTotal());
		...
		colChon.setCellFactory(param -> new TableCell<MonAnViewModel, Void>() {
			private final Button btnChon = new Button("Chọn");
			{
				btnChon.setOnAction(event -> {
					...
					if (monVM != null) {
						handleChonMon(monVM.getMaMon(), monVM.getTenMon(), monVM.getGiaBan()); // Truyền thông tin cần
																								// thiết
					}
				});
			}
	...
	private void handleLuuDatHang() {
		...
		if (selectedBanList.isEmpty() || !isBookingConfirmed) {
			/* ... show alert ... */ return;
		}
		...
		try {
			...
			if (sdt.isEmpty() || ngayDen == null || gioStr.isEmpty()) {
				/* ... show alert ... */ return;
			}
			...
			for (Ban banDuocChon : selectedBanList) {
				...
				if (trangThaiHienTai != TrangThaiBan.TRONG) {
					showAlert(Alert.AlertType.WARNING, "Bàn bận",
							"Bàn " + banDuocChon.getMaBan() + " vừa bị chiếm. Vui lòng chọn lại.");
					resetSelectionAndConfirmation();
					loadTableGrids();
					return;
				}
...
//                // Tiền cọc (đã parse và kiểm tra)
				double tienCoc = 0;
				try {
					String raw = txtTienCoc.getText().replaceAll("[^0-9.]", "");
					tienCoc = Double.parseDouble(raw.isEmpty() ? "0" : raw);
				} catch (Exception e) {
				}
...
//                datBanDAO.luuHoaDonVaChiTiet(newHoaDon, monOrderList);
				...
				String trangThaiBanDb = (trangThaiBanDau == TrangThaiHoaDon.DAT) ? TrangThaiBan.DA_DAT.getDbValue()
						: TrangThaiBan.DANG_SU_DUNG.getDbValue();
				datBanDAO.capNhatTrangThaiBan(banDuocChon.getMaBan(), trangThaiBanDb);
			}

			showAlert(Alert.AlertType.INFORMATION, "Thành công",
					"Đã lưu đặt hàng! Trạng thái: " + trangThaiBanDau.getDisplayName());
			resetSelectionAndConfirmation();
			loadBookingCards();
			loadTableGrids();
	...
	private void handleXacNhanBan() {
		/* ... Giữ nguyên logic tính cọc, đổi màu, set flag isBookingConfirmed ... */ }
...
//    private TrangThaiBan getTrangThaiHienThi(Ban banGoc, LocalTime thoiGianKiemTra) { /* ... Giữ nguyên ... */ }
	...
	private void handleChonBan(Ban ban, Button currentButton) {
		/* ... Giữ nguyên logic cảnh báo và chọn/bỏ chọn ... */ }
	...
	private void resetSelectionAndConfirmation() {
		...
		for (Button btn : selectedButtonList) {
			...
			if (maBan != null) {
				allBans.stream().filter(b -> b.getMaBan().equals(maBan)).findFirst().ifPresent(banGoc -> {
...
//    private VBox createBookingCard(HoaDon hd) { /* ... Giữ nguyên ... */ }
	...
	private void handleSelectBookingCard(HoaDon hd) {
		/* ... Giữ nguyên logic load dữ liệu lên form, tải món, ẩn/hiện panel ... */ }

	/** Reset màu các nút trên sơ đồ bàn */
	private void resetUIMapSelection() {
		/* ... Giữ nguyên ... */ }

	/** Tô màu nút bàn được chọn từ card */
	private void highlightSelectedTableOnMap(Ban selectedBan) {
		/* ... Giữ nguyên ... */ }

	/** Tìm món ăn theo keyword */
	private void handleTimMon(String keyword) {
		List<entity.MonAn> dsEntityMonAn = (keyword == null || keyword.trim().isEmpty()) ? monAnDAO.getAllMonAn()
	...
	private void loadMonAnTable(List<entity.MonAn> dsEntityMonAn) {
		List<MonAnViewModel> dsViewModel = dsEntityMonAn.stream().map(MonAnViewModel::fromEntity)
	...
	private void handleChonMon(String maMon, String tenMon, double giaBan) {
		...
		if (existingOrder.isPresent()) {
			...
			monOrderList.add(new MonOrder(maMon, tenMon, giaBan, 1));
		}
	...
	private void showOrHideReceiptPanel(boolean show) {
		/* ... Giữ nguyên ... */ }

	/** Xử lý khi nhấn nút Thanh Toán (giờ là hiển thị panel hóa đơn) */
	private void handleDisplayReceipt() {
		/* ... Giữ nguyên logic kiểm tra trạng thái và hiển thị panel ... */ }

	/** Xử lý khi nhấn nút Xác Nhận Thanh Toán (cuối cùng) */
	private void handleXacNhanThanhToan() {
		/* ... Giữ nguyên logic lấy PTTT, gọi DAO cập nhật, reset UI ... */ }
...
//         showAlert(Alert.AlertType.INFORMATION, "In hóa đơn", "Chuẩn bị in hóa đơn: " + currentHoaDon.getMaHD());
	...
	public static class MonAnViewModel {
		...
		public MonAnViewModel(String maMon, String tenMon, double giaBan, byte[] hinhAnhBytes) {
			...
			if (hinhAnhBytes != null && hinhAnhBytes.length > 0) {
				try (ByteArrayInputStream bis = new ByteArrayInputStream(hinhAnhBytes)) {
					...
					System.err.println("Lỗi load ảnh món ăn: " + e.getMessage());
				}
			}
		...
		public SimpleDoubleProperty giaBanProperty() {
			return giaBan;
		}

		public SimpleObjectProperty<Image> hinhAnhProperty() {
			return hinhAnh;
		}
	}
	...
	public static class MonOrder {
		...
		public MonOrder(String maMon, String tenMon, double donGia, int soLuong) {
			...
			this.soLuong.addListener((obs, oldV, newV) -> thanhTien.set(donGiaProperty().get() * newV.intValue()));
		}
