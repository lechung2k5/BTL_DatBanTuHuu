...
public class NhanVienUI {
	...
	@FXML
	private ComboBox<String> cbxCaLam; // Sẽ dùng cho ca YÊU THÍCH
	@FXML
	...
	@FXML
	private TableColumn<NhanVien, String> colCaLam; // (FXML id vẫn là colCaLam)
	@FXML
	private TableColumn<NhanVien, String> colTrangThai;

	// Khai báo đúng 6 nút
	@FXML
	private Button btnThem; // Nút "Thêm" (Lưu mới)
	@FXML
	private Button btnSua; // Nút "Sửa" (Cập nhật ngay)
	@FXML
	private Button btnXoa;
	@FXML
	private Button btnLuu; // Nút "Lưu" (Không dùng)
	@FXML
	private Button btnHuy;
	@FXML
	private Button btnXoaRong; // Nút "Xóa rỗng" (Bắt đầu thêm)
	// </editor-fold>
	...
	private void initialize() {
		...
		for (VaiTro vaiTro : VaiTro.values()) {
			vaiTroStrings.add(vaiTro.getTenVaiTro());
		}
		...
		tblNhanVien.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
			if (currentState == EditState.ADDING)
				return; // Không làm gì nếu đang thêm
	...
	private void handleThem(ActionEvent event) { // Nút "Thêm" -> Lưu nhân viên mới
		if (currentState != EditState.ADDING || !validateInput())
			return;
	...
	private void handleSua(ActionEvent event) { // Nút "Sửa" -> Cập nhật ngay dòng đang chọn
		if (currentSelectedNhanVien == null || currentState != EditState.VIEWING || !validateInput())
			return;
	...
	private void handleXoa(ActionEvent event) { // Nút "Xóa"
		if (currentSelectedNhanVien == null || currentState != EditState.VIEWING)
			return;
