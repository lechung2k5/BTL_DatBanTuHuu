...
public class DashboardController {
	...
	// Giữ nguyên các @FXML
	@FXML
	private GridPane weeklyCalendarGrid; // Đảm bảo fx:id khớp với FXML
	@FXML
	private DatePicker datePicker;
	@FXML
	private TextField txtSoTienKiemKe;
	@FXML
	private GridPane tableMapGrid;
	@FXML
	private Canvas hoursCanvas;
	@FXML
	private Canvas revenueCanvas;
	@FXML
	private Label hoursDetailsLabel;
	@FXML
	private Label revenueDetailsLabel;
	...
	// private final NhanVienDAO nhanVienDAO = new NhanVienDAO(); // Không cần thiết
	// nếu CaTrucDAO đã lấy tên NV
	...
	private void updateWeeklyCalendar() {
		...
		if (currentUser == null || currentUser.getNhanVien() == null) {
			// Xử lý trường hợp không có người dùng đăng nhập (hiển thị lịch trống hoặc
			// thông báo)
	...
	private LocalTime getTimeSlotEndTime(String timeSlot) {
		...
		case "tối":
			return LocalTime.of(2, 0); // Ca tối có thể qua ngày hôm sau
		default:
	...
	private void setupTableMap() { /* ... Giữ nguyên code cũ ... */
		...
		ObservableList<Table> tables = FXCollections.observableArrayList(new Table("Bàn 1", "Trống"),
				new Table("Bàn 2", "Đang phục vụ"), new Table("Bàn 3", "Trống"), new Table("Bàn 4", "Đã đặt"),
		...
		for (Table table : tables) {
			...
			case "đang phục vụ":
				tableLink.setStyle("-fx-background-color: #e74c3c;");
				break; // Đỏ
			case "đã đặt":
				tableLink.setStyle("-fx-background-color: #f39c12;");
				break; // Cam
			default:
				tableLink.setStyle("-fx-background-color: #2ecc71;");
				break; // Xanh lá (Trống)
			}
	...
	private void drawHoursChart() { /* ... Giữ nguyên code cũ ... */
		...
		double w = hoursCanvas.getWidth(), h = hoursCanvas.getHeight(), cx = w / 2, cy = h / 2,
				or = Math.min(w, h) / 2.5, ir = or * 0.7;
		gc.clearRect(0, 0, w, h);
		double ratio = 160.0 / 180.0;
		gc.setFill(Color.web("#e0e0e0"));
		gc.fillOval(cx - or, cy - or, 2 * or, 2 * or);
		gc.setFill(Color.web("#27ae60"));
		gc.fillArc(cx - ir, cy - ir, 2 * ir, 2 * ir, 90, -ratio * 360, javafx.scene.shape.ArcType.ROUND);
	}

	private void drawRevenueChart() { /* ... Giữ nguyên code cũ ... */
		GraphicsContext gc = revenueCanvas.getGraphicsContext2D();
		double w = revenueCanvas.getWidth(), h = revenueCanvas.getHeight(), cx = w / 2, cy = h / 2,
				or = Math.min(w, h) / 2.5, ir = or * 0.7;
		gc.clearRect(0, 0, w, h);
		double ratio = 5000000.0 / 6000000.0;
		gc.setFill(Color.web("#e0e0e0"));
		gc.fillOval(cx - or, cy - or, 2 * or, 2 * or);
	...
	private void setupTooltips() { /* ... Giữ nguyên code cũ ... */
		hoursCanvas.setOnMouseMoved(e -> {
			tooltip.setText("Số giờ làm: 160/180h");
		...
		revenueCanvas.setOnMouseMoved(e -> {
			tooltip.setText("Doanh thu: 5M/6M");
